...
const go = prompt(chalk`{yellow Did you create an application at} {blue https://discord.com/developers/applications}{yellow ? If not, you probably should read the} {green README} {yellow on the GitHub repo before continuing. Enter} {green 'y'} {yellow or} {green 'yes'} {yellow to continue.\n}`).toLowerCase();
...
fs.writeFile(`${dir}/drg-options.json`, data, (err) => {
	if (err) { throw err; }
	console.log(chalk`\n{bold.magenta Done!} Time to run the {cyan presence.js} file, instructions in the {green README}.\nI really hope you enjoy using this! If you do, please consider {bold.yellow starring this repository on GitHub}: {blue https://github.com/ThatOneCalculator/DiscordRPCGenerator}`)
});
